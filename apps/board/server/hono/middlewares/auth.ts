...
const isSessionProfile = (profile: unknown): profile is SessionUser['profile'] =>
  !!profile &&
...
const isSessionUser = (payload: JWTPayload): payload is JWTPayload & SessionUser =>
...
export const optionalAuth: MiddlewareHandler<AppEnv> = async (context, next) => {
  ...
  try {
    ...
    if (!isSessionUser(payload)) throw new Error('Invalid session payload')
